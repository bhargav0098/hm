  ...
  if (loading) {
    return (
      <div className="glass-card p-5 h-64 flex flex-col items-center justify-center">
        ...
        <p className="text-white/50 text-sm">Finding your Career Twin...</p>
      </div>
    );
  }
  ...
  if (!twin) {
    return (
      <div className="glass-card p-5">
        <p className="text-white/50">Please complete your profile to see your Career Twin.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 mb-5 relative z-10">
        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-space-border to-space-border border-2 border-primary-500/40 flex items-center justify-center flex-shrink-0">
      ...
      <div className="space-y-4 relative z-10">
        <div>
          <p className="text-white/50 text-xs font-semibold mb-2">Missing Skills to Match:</p>
        ...
        <div>
          <p className="text-white/50 text-xs font-semibold mb-2">Their Successful Path:</p>
          <ul className="space-y-2 text-xs text-white/50">
             {(twin.successfulPath || []).map((step, i) => (
               <li key={i} className="flex items-start gap-2">
                 <CheckCircle className="w-3.5 h-3.5 text-accent-green flex-shrink-0 mt-0.5" />
                 <span><strong className="text-white/50">Step {step.step}:</strong> {step.description}</span>
               </li>
             ))}
          </ul>
        </div>
        ...
        <div>
          <p className="text-white/50 text-xs font-semibold mb-2">Your Next Actions:</p>
          <ul className="space-y-2 text-xs text-white/50">
